...
import static defaultplayer_intlqualifier.util.CheckWrapper.contains;
...
public class Optimizer {
    ...
    public static MapLocation nearestSpawnZone(RobotController rc, MapLocation center) {
        int minDistance = Integer.MAX_VALUE;
        MapLocation nearest = null;
        for (MapLocation home : Constants.SPAWN_ZONES) {
            if (center.distanceSquaredTo(home) < minDistance) {
                nearest = home;
                minDistance = center.distanceSquaredTo(home);
            }
        }
        for (Direction dir : DIRECTIONS) {
            if (contains(SPAWN_ZONE_CENTERS, nearest.add(dir))) return nearest.add(dir);
        }
        return nearest;
    }
